...
namespace PC.Shared
{
    ...
    internal static class Ensure
    {
        ...
        /// <exception cref="Exception">Exception thrown if the specified condition is not met</exception>
        ...
        /// Ensures that the given expression is true
        /// </summary>
        /// <typeparam name="TException">Type of exception to throw when the specified condition is not met</typeparam>
        /// <param name="condition">Condition to test/ensure</param>
        /// <param name="message">Message for the exception</param>
        /// <exception>
        ///     A {TException} will be thrown when <paramref name="condition"/> is false
        /// </exception>
        /// <remarks>{TException} must have a constructor that takes a single string parameter</remarks>
        ...
        /// <param name="condition">Condition to test/ensure</param>
        /// <param name="message">Message for the exception</param>
        /// <param name="paramName">Parameter name being tested</param>
        /// <exception>
        ///     A {TException} will be thrown when <paramref name="condition"/> is false
        /// </exception>
        /// <remarks>{TException} must have a constructor that takes a single string parameter</remarks>
        public static void That<TException>(bool condition, string message, string paramName) where TException : ArgumentException
        {
            if (!condition)
                throw (TException)Activator.CreateInstance(typeof(TException), message, paramName);
        }

        /// <summary>
        /// Ensures that the given expression is false
        /// </summary>
        /// <typeparam name="TException">Type of exception to throw when the specified condition is met</typeparam>
        /// <param name="condition">Condition to test/ensure</param>
        /// <param name="message">Message for the exception</param>
        /// <exception>
        ///     A {TException} will be thrown when <paramref name="condition"/> is true
        /// </exception>
        /// <remarks>{TException} must have a constructor that takes a single string parameter</remarks>
        ...
        public static class Argument
        {
            ...
            {
                That<ArgumentNullException>(value != null, paramName);
                That<ArgumentException>(!string.Empty.Equals(value), "String value cannot be empty", paramName);
            }
        }
    }
}